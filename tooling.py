...
class IngestFacadeProtocol(Protocol):
    async def ingest_local_file(self, source_path: str, group_name: str, description: str = "", preferred_name: str | None = None): ...
    async def save_remote_image(self, image_url: str, group_name: str, description: str = "", preferred_name: str | None = None, source: str = "remote"): ...
...
class StealMemesTool(FunctionTool):
    ...
    parameters: dict = field(default_factory=lambda: {
        "type": "object",
        "properties": {
            "image_path": {"type": "string", "description": "本地图片绝对路径、相对路径或 http/https 图片地址。"},
            "category": {"type": "string", "description": "必填，要保存到的图片分组名。"},
            "description": {"type": "string", "description": "可选，分组的中文用途描述。"},
            "save_name": {"type": "string", "description": "可选，保存后的文件名，不含路径。"}
        },
        "required": ["image_path", "category"]
    })

    async def call(self, context: Any, image_path: str, category: str, description: str | None = None, save_name: str | None = None) -> str:
        _ = context
    ...
    async def run(self, event: AstrMessageEvent, image_path: str, category: str, description: str | None = None, save_name: str | None = None) -> str:
        _ = event
        return await self.call(context=None, image_path=image_path, category=category, description=description, save_name=save_name)
