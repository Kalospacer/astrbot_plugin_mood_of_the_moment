...
**每轮 Prompt 注入（冷门优先、动态轮换、不随机）：**
- 精确表情定义：当前库内全部 `meme_def`（受 `max_prompt_meme_defs` 限制），按**冷门优先**排序——从未用过的排最前，被用一次即让位给下一批，形成动态轮换，避免大模型反复点名同一批表情包
- 分组标签：当前库内全部 `tags`（受 `max_prompt_tags` 限制），按"该 tag 下最冷门资产"排序，冷门 tag 前置
- 使用规则提示

**发送解析顺序：**
1. marker 单 token 且命中 `meme_def` → 精确发送唯一图片；
2. 未命中 `meme_def` → 解释为 tag，单 tag 或组合 tag 走降级评分；
3. 同分时优先**最久未用、使用次数最少**的表情包，再按 `meme_def` 字典序稳定兜底（不引入随机）；
4. 完全未命中 → 静默删除 marker。
